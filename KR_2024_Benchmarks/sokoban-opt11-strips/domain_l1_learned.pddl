(define (domain sokoban-sequential)
(:requirements :strips :typing :negative-preconditions :equality)
  (:types
    thing - object
    location - object
    direction - object
    player - thing
    stone - thing
  )

  (:predicates
    (at ?t - thing ?l - location)
    (at-goal ?s - stone)
    (clear ?l - location)
    (is-goal ?l - location)
    (is-nongoal ?l - location)
    (move-dir ?from - location ?to - location ?dir - direction)
  )
  (:action move
    :parameters (?location1 - location ?location2 - location ?player1 - player)
    :precondition (and
      (at ?player1 ?location2)
      (clear ?location1)
    )
    :effect (and
      (at ?player1 ?location1)
      (clear ?location2)
      (not(at ?player1 ?location2))
      (not(clear ?location1))
    )
  )

  (:action push-to-goal
    :parameters (?location1 - location ?location2 - location ?location3 - location ?player1 - player ?stone1 - stone)
    :precondition (and
      (at ?player1 ?location1)
      (at ?stone1 ?location2)
      (clear ?location3)
      (is-goal ?location3)
    )
    :effect (and
      (at ?player1 ?location2)
      (at ?stone1 ?location3)
      (at-goal ?stone1)
      (clear ?location1)
      (not(at ?player1 ?location1))
      (not(at ?stone1 ?location2))
      (not(clear ?location3))
    )
  )

  (:action push-to-nongoal
    :parameters (?location1 - location ?location2 - location ?location3 - location ?player1 - player ?stone1 - stone)
    :precondition (and
      (at ?player1 ?location1)
      (at ?stone1 ?location2)
      (clear ?location3)
      (is-nongoal ?location3)
    )
    :effect (and
      (at ?player1 ?location2)
      (at ?stone1 ?location3)
      (clear ?location1)
      (not(at ?player1 ?location1))
      (not(at ?stone1 ?location2))
      (not(at-goal ?stone1))
      (not(clear ?location3))
    )
  )

)
